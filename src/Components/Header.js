  ...
  return (
    <>
      ...
      <header className="fixed top-4 left-4 right-4 z-50 max-w-7xl mx-auto">
        <div className="bg-bg-card/90 backdrop-blur border border-border-subtle rounded-xl px-5 py-3 md:px-6 md:py-4">
          <div className="flex justify-between items-center">
            <NavLink to="/" className="name-logo no-underline">
              Alexandre <span>Benoit</span>
            </NavLink>
            <button
              type="button"
              onClick={toggleClass}
          ...
          <nav
            className={`${!isOpen ? "hidden" : "block"} md:block mt-4 md:mt-0`}
          >
            <div className="md:flex md:justify-between md:items-center">
              <ul className="md:flex md:items-center md:space-x-6">
                {navLinks.map(({ to, label }) => (
                  <li key={to} className="py-2 md:py-0">
                    <NavLink to={to} onClick={toggleClass} className={navClass}>
                      {label}
                    </NavLink>
                  </li>
                ))}
              </ul>
              <ul className="flex justify-center items-center gap-4 mt-4 md:mt-0 md:ml-6">
                <li>
                  <a
                    href={linkdein}
                    target="_blank"
                    rel="noreferrer noopener"
                    className="cursor-pointer text-text-muted hover:text-accent-cyan transition-colors duration-200"
                    aria-label="LinkedIn"
                  >
                    <svg className="fill-current w-6 h-6" viewBox="0 0 30 30">
                ...
                <li>
                  ...
                  >
                    <svg className="fill-current w-6 h-6" viewBox="0 0 50 50">
                      <path
                        fillRule="nonzero"
                        clipRule="nonzero"
